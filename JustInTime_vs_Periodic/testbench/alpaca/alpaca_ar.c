...
__GLOBAL_SCALAR(uint16_t,         i_debug);
__GLOBAL_SCALAR(uint16_t,         ar_v_seed);

__GLOBAL_SCALAR(uint16_t,         _v_count);
__GLOBAL_SCALAR(uint16_t,         _v_pinState);
__GLOBAL_SCALAR(uint16_t,         _v_samplesInWindow);
__GLOBAL_SCALAR(uint16_t,         _v_totalCount);
__GLOBAL_SCALAR(uint16_t,         _v_movingCount);
__GLOBAL_SCALAR(uint16_t,         _v_stationaryCount);
__GLOBAL_SCALAR(uint16_t,         _v_discardedSamplesCount);
__GLOBAL_SCALAR(uint16_t,         _v_trainingSetSize);

__GLOBAL_SCALAR(ar_class_t,      _v_class);
__GLOBAL_SCALAR(ar_run_mode_t,   _v_mode);
__GLOBAL_SCALAR(ar_features_t,   _v_features);
__GLOBAL_SCALAR(uint16_t,         _v_meanmag);
__GLOBAL_SCALAR(uint16_t,         _v_stddevmag);

__GLOBAL_ARRAY(accelReading,    _v_window,AR_ACCEL_WINDOW_SIZE);
__GLOBAL_ARRAY(ar_features_t,   _v_model_stationary,AR_MODEL_SIZE);
__GLOBAL_ARRAY(ar_features_t,   _v_model_moving,AR_MODEL_SIZE);

__GLOBAL_ARRAY(uint16_t,         resultStationaryPct,4);
...
__TASK(3, AR_Transform);  //_v_window[]
  ...
  /*         if (!vbm_test(_v_window_vbm[i].y)) {
       ...
       __TRANSITION_TO(4, AR_Featurize);
       ...
       case MODE_TRAIN_MOVING:
           ...
           __TRANSITION_TO(8, AR_Train);
       case MODE_RECOGNIZE:
           __GET(_v_features.meanmag) = features.meanmag;
           __GET(_v_features.stddevmag) = features.stddevmag;
           __TRANSITION_TO(5, AR_Classify);
       }
