...
__SHARED_VAR(
 ...
 uint32_t  index2; //2
 unsigned  n; //1 
 uint32_t  input[2]; //2*2=4
 unsigned char  iv[8]; //1/2)*8=4
...
static const uint16_t global_war_size = 12;
static __nv uint16_t  backup_buf[12] = {};
...
void pc_blowfish_main()
{
	...
	__BUILDIN_TASK_BOUNDARY(0,  task_init);
	...
	__BUILDIN_TASK_BOUNDARY(1, task_set_ukey);
	...
	if (__GET(index2) == 0) {
		...
		if (__GET(index2) < 20) { //set key
			__GET(key[__GET(index2)-2]) = __GET(input[0]);
			__GET(key[__GET(index2)-1]) = __GET(input[1]);
			...
			if (__GET(index2) < (256 + 20)) { //set s0 
				__GET(s0[__GET(index2)-20]) = __GET(input[0]);
				__GET(s0[__GET(index2)-19]) = __GET(input[1]);
				...
				__GET(s1[__GET(index2)-(256+19)]) = __GET(input[1]);
				...
				__GET(s2[__GET(index2)-(256*2+19)]) = __GET(input[1]);
				...
				__GET(s3[__GET(index2)-(256*3+19)]) = __GET(input[1]);
    ...
    c = indata[__GET(index2)]^(__GET(iv[__GET(n)]));

	__GET(result[__GET(index2)]) = c;
	
	__GET(iv[__GET(n)]) = c;

	__GET(n) = (__GET(n)+1)&0x07;
	++__GET(index2);
	if (__GET(index2) == LENGTH) {
        __FINISH;
		//__NEXT(2, task_done);	
	}
	else {
