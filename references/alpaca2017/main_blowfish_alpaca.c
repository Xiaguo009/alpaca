...
void init()
{
	...
	*timer |= 0x0002; //interrupt enable
	...
	ENTRY_TASK(task_init)  //_entry_task(func of task_t)   _task_entry_task(task_t)
 		//TASK(0, _entry_task) //void _entry_task(); __nv task_t _task_entry_task = {  _entry_task , idx  };  
		//= void _entry_task() { TRANSITION_TO(&_task_task_init); }
INIT_FUNC(init)
