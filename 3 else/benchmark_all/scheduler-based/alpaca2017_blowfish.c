...
#if VERBOSE > 0
				if (index2_priv == 20 || index2_priv == 254 + 20) {
					LOG("s0[%u]=",index2_priv-20);
					print_long(GV(input, 0));
					//print_long(k0);
					LOG("s0[%u]=",index2_priv-19);
					print_long(GV(input, 1));
					//print_long(k1);	
				}
#endif
...
#if VERBOSE > 0
				if (index2_priv == 256 + 20 || index2_priv == (256*2-2) + 20) {
					LOG("s1[%u]=",index2_priv-(256+20));
					print_long(GV(input, 0));
					//print_long(k0);
					LOG("s1[%u]=",index2_priv-(256+19));
					print_long(GV(input, 1));
					//print_long(k1);	
				}
#endif
...
#if VERBOSE > 0
				if (index2_priv == 256*2 + 20 || index2_priv == (256*3-2) + 20) {
					LOG("s2[%u]=",index2_priv-(256*2+20));
					print_long(GV(input, 0));
					//print_long(k0);
					LOG("s2[%u]=",index2_priv-(256*2+19));
					print_long(GV(input, 1));
					//print_long(k1);	
				}
#endif
...
#if VERBOSE > 0
				if (index2_priv == 256*3 + 20 || index2_priv == (256*4-2) + 20) {
					LOG("s3[%u]=",index2_priv-(256*3+20));
					print_long(GV(input, 0));
					//print_long(k0);
					LOG("s3[%u]=",index2_priv-(256*3+19));
					print_long(GV(input, 1));
					//print_long(k1);	
				}
#endif
...
void task_encrypt() {
	...
	//l = GV(input, 1);
    //rd
	if (!vbm_test(input_vbm[0])) {input_priv[0] = GV(input, 0);}
	r = input_priv[0];
	if (!vbm_test(input_vbm[1])) {input_priv[1] = GV(input, 1);}
	l = input_priv[1];
	...
	if (!vbm_test(input_vbm[1])) {
		vbm_set(input_vbm[1]);
		write_to_gbuf(&input_priv[1], &input[1], sizeof(input[1])); //GV?}
	
	input_priv[0] = l;
	if (!vbm_test(input_vbm[0])) {
		vbm_set(input_vbm[0]);
		write_to_gbuf(&input_priv[0], &input[0], sizeof(input[0])); //GV?}
...
//warset: n index2 pass  iv[8]  pass
void task_start_encrypt3() {
	...
	unsigned char c;
    //rd
	//c = indata[index2_priv]^(GV(iv, _global_n));
    if (!vbm_test(iv_vbm[_global_n])) {iv_priv[_global_n] = GV(iv, _global_n);}
    c = indata[index2_priv]^(iv_priv[_global_n]);
	...
	//GV(iv, _global_n) = c;
    iv_priv[_global_n] = c;
    if (!vbm_test(iv_vbm[_global_n])) {
        iv_priv[_global_n] = GV(iv, _global_n);
        write_to_gbuf(&iv_priv[_global_n], &iv[_global_n], sizeof(iv[_global_n])); //GV?}
